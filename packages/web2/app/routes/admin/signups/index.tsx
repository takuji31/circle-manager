...
import { getNotJoinedSignUps, inviteMember } from "~/model/signup.server";
          ...
          {notInvitedSignUps.length ? (
            <SignUpList
              children={notInvitedSignUps.map((signUp) => (
                <SignUpListItem key={signUp.id} signUp={signUp} />
              ))}
            />
          ) : (
            <p>勧誘待ちはありません</p>
...
const SignUpListItem: React.FC<{ signUp: SignUp }> = ({ signUp }) => {
