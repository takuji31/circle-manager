...
import { isValidMonthSurveyEmoji, monthSurveyAnswerLabel, MonthSurveyEmoji } from "@/model";
import type { MonthSurvey } from "@prisma/client";
import { MemberStatus, MonthSurveyAnswerValue } from "@prisma/client";
import type { ReactionHandlerWithData } from "./types";
...
export const monthSurveyShowReaction: ReactionHandlerWithData<MonthSurvey> = async (reaction, user, _, data) => {
  await reaction.users.remove(user.id);
